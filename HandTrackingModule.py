...
class handDetector():
  ...
  def findPosition(self, img, handNo=0, draw = True):
    ...
    if self.results.multi_hand_landmarks:
      ...
      if draw:
        cv2.rectangle(img, (xmin - 20, ymin - 20), (xmax + 20, ymax + 20), (255,0,8), 2)
  ...
  def fingersUp(self):
    ...
    # For Thumb
    if self.lmList[self.tipHands[0]][1] < self.lmList[self.tipHands[0]-1][1]:
      fingers.append(0)
    else:
      fingers.append(1)
    ...
    # For Fingers
    for id in range(1, 5):
      if self.lmList[self.tipHands[id]][2] < self.lmList[self.tipHands[id]-2][2]:
        fingers.append(1)
      else:
        fingers.append(0)
  ...
  def findDistance(self, p1, p2, img, draw = True, r = 10, t = 2):
    ...
    if draw:
      cv2.circle(img, (x1, y1), r, (255, 0, 8), cv2.FILLED)
      cv2.circle(img, (x2, y2), r, (255, 0, 8), cv2.FILLED)
      cv2.circle(img, (cx, cy), r, (255, 0, 255), cv2.FILLED)
      cv2.line(img, (x1, y1), (x2, y2), (255, 0 , 8), t)
